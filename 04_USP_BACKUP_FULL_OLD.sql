...
INSERT INTO [IT2_SysAdmin].[dbo].[versioncheck] ([tab_proc_name], version, created, procvers) VALUES ('USP_BACKUP_FULL','$(pstdvers)',GETDATE(),'1.11')
ELSE
UPDATE IT2_SysAdmin.dbo.versioncheck SET procvers = '1.11', modified = GETDATE() WHERE tab_proc_name = 'USP_BACKUP_FULL'
GO
PRINT '---------------------------------------
...
-- ------------------------------------------------------------------------------------------------
  ...
  AS
   ...
   IF @db_name = ''
      BEGIN -- BeginA
		...
		IF (SELECT  substring(CONVERT(sysname,SERVERPROPERTY('ProductVersion')),0,5)) LIKE '11.%'
 ...
 -- ------------------------------------------
		...
		DECLARE cur_rodb  CURSOR STATIC LOCAL /* Cursor für die Read-Only-DBs */
		 FOR SELECT name FROM sys.databases 
			WHERE state = 0 /* state 0 = online -- cursor will get every db that can be backuped */
			AND name <> 'tempdb' /* Tempdb muss nicht gesichert werden*/
